 ...
 */
async function main() {
  const batch = AUTHORED_QUESTIONS.filter((q) => q.authoredId.startsWith("NG-BATCH2-") || q.authoredId.startsWith("NG-USER-"));
  if (batch.length !== 50) throw new Error(`Expected 70 appended questions, found ${batch.length}`);
