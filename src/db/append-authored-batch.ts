 ...
 */
async function main() {
  const batch = AUTHORED_QUESTIONS.filter((q) => q.authoredId.startsWith("NG-BATCH2-") || q.authoredId.startsWith("NG-USER-") || q.authoredId.startsWith("NG-NEURO-") || q.authoredId.startsWith("NG-FLUID-") || q.authoredId.startsWith("NG-PHARM-"));
  if (batch.length !== 50) throw new Error(`Expected 130 appended questions, found ${batch.length}`);
